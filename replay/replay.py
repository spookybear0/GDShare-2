...
def import_replay(file: str):
    file.unpack_replay_file(file)

def export_replay():
    file.pack_replay_file(recent, f"{user_name} - {level.creator_name} - {level.name}.gdr")

def record_input(): # internal
    global play_layer
    recorded = []
...
def play_input(input: list): # internal
...
def play_replay(key, input=None):
    if not input:
        ...
        global start_time
        if not recent:
            recent = file.unpack_replay_file("recent")
        playing_replay = True
    else:
        data = []
        for i in input:
    ...
    while started:
        ...
        if scene == Scene.EDITOR_OR_LEVEL or scene == Scene.OFFICIAL_LEVEL:
            ...
            if play_layer.percent > 0:
                ...
                print("Recording...")
                recent = record_input()
                file.pack_replay_file(recent, "recent")
                print("Done")
